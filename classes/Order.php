	...
	public function create()
{
	...
	if( !empty($type)&&
		...
		$exist = $this->_db->select("SELECT type FROM services WHERE type = '{$type}'")->results();
	    
	     if(count($exist)>0)
	     {
	     	 echo "<h2>Service already in exists</h2>";
	     } else{

	     	  //insert data with insert functon(functions/db-helper)
			if($this->_db->insert('services',[
				    'type'=>ucwords($type),
				    'rate'=>$rate,
				    'date_created'=>$date

				])){
				echo "<h2 style='color:red'>Service successfully created</h2>";
			}
			else{
				echo "<h2 style='color:red'>Sorry, could not create service!</h2>";
			}
	     }
...
public function add()
{
	...
	if(!empty($prodId)&&!empty($quantity)){
	 ...
	 $exist = $this->_db->select("SELECT * FROM products WHERE id = '{$prodId}'")->first();
		//print_array($exist); die();
	    $qty = $exist->quantity + $quantity;

		
		if($this->_db->update('products', [

			      'quantity'=>$qty], $exist->id
			      )){
...
public function get($id="*")
{
	if($id=="*"){
      ...
      return $this->_db->select("SELECT customers.name, orders.* FROM `customers`, `orders` WHERE customers.customer_id = orders.customer_id ORDER BY `placed_at` DESC")->results();
	}
	else{
		//select a particular product
		return $this->_db->select("SELECT * FROM  `orders` WHERE customer_id = '{$id}' ORDER BY `placed_at` DESC")->results();
	}
}
...
public function update()
{
	...
	if($this->_db->update2('services', ['id'=>$serviceId], [
		       'type'=>$type,
		       'rate'=>$rate,
		       'date_created'=>$date
		])){
...
public function delete()
{
	 ...
	 if(count($itemSold)>0) {
	 	return false;
	 	 ...
	 	 if($this->_db->delete('services', ['id','=',$serviceId])) {
		 	return true;
		 } else{
		 	return false;
		 }
