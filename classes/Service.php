...
public function get($id="*")
{
	if($id=="*"){
		//select all products
      return $this->_db->select("SELECT * FROM `services` ORDER BY `type` ASC")->results();
	}
	else{
		//select a particular product
		return $this->_db->select("SELECT * FROM `services` WHERE `service_id`='$id'")->first();
	}
}
...
public function update()
{
	...
	if($this->_db->update2('services', ['service_id'=>$serviceId], [
		       'type'=>$type,
		       'rate'=>$rate,
		       'date_created'=>$date
		])){
...
public function delete()
{
	 ...
	 if(count($itemSold)>0) {
	 	return false;
	 	 ...
	 	 if($this->_db->delete('services', ['service_id','=',$serviceId])) {
		 	return true;
		 } else{
		 	return false;
		 }
