...
function createProduct()
{
	...
	if( !empty($name)&&
		...
		$itemInStock = select("SELECT name, price FROM products WHERE name = '{$name}' AND price = '{$price}'");
...
function addProduct()
{
	...
	if(!empty($prodId)&&!empty($quantity)){
		...
		$itemInStock = select("SELECT * FROM products WHERE id = '{$prodId}'")[0];
	    $qty = $itemInStock['quantity'] + $quantity;

		//insert data with insert functon(functions/db-helper)
		if(update('products', ['id'=>$prodId], [
...
{
	 $prodId      = $_POST['prod-id'];
	 $itemSold   = select("SELECT id FROM sales WHERE product_id = '{$prodId}'");
	 if(count($itemSold)>0) {
	 	return false;
	 } else {

	 	 if(delete('products', $prodId)) {
		 	return true;
		 } else{
		 	return false;
		 }
...
function allowSale($qtyBought, $prodId)
{
	$stock = displayProd($prodId);
	
	if($stock[0]['quantity']>=$qtyBought) {
		return true;
	} else{
		return false;
	}
...
function trackSales(array $sales)
{
...
{ 
   ...
   if(isset($_SESSION['prod-id'])/*&&isset($_SESSION['qty-in-stock'])*/){
		   ...
		   if(allowSale($qtyBought, $prodId))
		   {  
		       //reduce quantity available by quantity just sold
		   	   //$_SESSION['qty-in-stock'] = $_SESSION['qty-in-stock'] - $qtyBought;
               if(update('products',['id'=>$prodId], ['quantity'=>$qtyBought],'-')&&
               	  update('products',['id'=>$prodId], ['qty_sold'=>$qtyBought],'+')) {
    		...
    		if(update('products',['id'=>$sales['prod_id']], [
                 ...
                 //handle credit sales perculiarities
			   	 if(isset($_POST['accept-credit-sale'])) {
                      ...
                      //get customer id
		              $_SESSION['customer-id']  =  mysql_insert_id();
		          }
					   	 insert('sales',[
					   	 	     'time'=>$time,
			   ...
			   if($iterations==count($_SESSION['sales'])) {
			   	//set current sales to zero
			   	 foreach($_SESSION['sales'] as $sales) {
			   	 	update('products', ['id'=>$sales['prod_id']], ['current_sale'=>0]);
			   	 }
			   	  return true;
			   }
    	}
...
function cancelSales()
{
	if(isset($_SESSION['sales'])) {
		...
		foreach($sales as $sale) {
            $product = displayProd($sale['prod_id']);
    		$qtyInStock = $product[0]['quantity'];
    		//reverse sales transactions
    		$qtyLeft  = $product[0]['quantity'] + $sale['quantity'];
    		$qtySold  = $product[0]['qty_sold'] - $sale['quantity'];
			if(update('products',['id'=>$sale['prod_id']], [
			   	          'quantity'=>$qtyLeft,
			   	          'qty_sold'=>$qtySold     
			   	])) {
				$iterations++;
...
{ 
	...
	//$sellerId = $_SESSION['ADMINID'];
	if($date == "all") {
		return select("SELECT products.*, sales.* FROM products, sales WHERE products.id = sales.product_id ORDER BY sales.date_sold DESC");
	} else {
		$today = date('Y-m-d');
	    if(empty($date))
        $date = $today;
	     //selecting from the products and sales tables
         return select("SELECT products.*, sales.* FROM products, sales WHERE products.id = sales.product_id AND sales.date_sold='$date' ORDER BY sales.time");
	}
...
*@param
*/
function getSalesByRange($from=null, $to=null, $prodId)
{
	...
	if($from && $to) {
	   ...
	   if($prodId == "all") {
		 return select("SELECT * FROM `sales` WHERE `date_sold` BETWEEN '{$from}' AND '{$to}'");
	   } else {

	   	   return select("SELECT * FROM `sales` WHERE `date_sold` BETWEEN '{$from}' AND '{$to}' AND `product_id`='{$prodId}'");
	   }
...
function getOutstanding($from=null, $to=null, $prodId)
{
     $outstanding = 0;
	 $data = [];
	if($from && $to) {
	   ...
	   if($prodId == "all") {
		   $data = select("SELECT * FROM `customers` WHERE `date` BETWEEN '{$from}' AND '{$to}' AND `owing` != 0");
	   } else {

	   	   $data = select("SELECT * FROM `customers` WHERE `date` BETWEEN '{$from}' AND '{$to}' AND `owing` != 0 AND `product_id`='{$prodId}'");
	   }
...
function getExpense($from=null, $to=null)
{ 
	//gurantee valid date format
	    $expense = 0;
		$date_format = new DateTime();
		$from = $date_format->format($from);
		$to   = $date_format->format($to);
        $expenses =  select("SELECT * FROM `expenses` WHERE `date` BETWEEN '{$from}' AND '{$to}'");
...
{
	return select("SELECT * FROM `expenses`");
}
